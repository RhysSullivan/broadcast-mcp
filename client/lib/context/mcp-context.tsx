...
async function waitForServerReady(
  ...
  console.log(
    `Checking server readiness at ${url}, will try ${maxAttempts} times`
  );
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      console.log(
        `Server not ready yet (attempt ${i + 1}), status: ${response.status}`
      );
    } catch (error) {
      console.log(
        `Server connection failed (attempt ${i + 1}): ${
...
export function MCPProvider({ children }: { children: React.ReactNode }) {
  ...
  const getActiveServersForApi = (): MCPServerApi[] => {
    return selectedMcpServers
      ...
      .map((server) => ({
        type: server.type === "stdio" ? "sse" : server.type,
        url:
          server.type === "stdio" && server.sandboxUrl
            ? server.sandboxUrl
            : server.url,
  ...
  const startServer = async (serverId: string): Promise<boolean> => {
    ...
    try {
      // For SSE and Web servers, just check if the endpoint is available
      if (server.type === "sse" || server.type === "web") {
      ...
      // For stdio servers, start the sandbox
      if (server.type === "stdio") {
        try {
          const { url: sandboxUrl } = await startSandbox({
            id: serverId,
          ...
          console.log(
            `Started sandbox for server ${serverId} at ${sandboxUrl}`
          );

          // Update server with sandbox URL
          const updatedServers = mcpServers.map((s) =>
            s.id === serverId ? { ...s, sandboxUrl } : s
          );
          ...
          console.error(
            `Error starting sandbox for server ${serverId}:`,
            error
          );
          updateServerStatus(
            serverId,
    ...
    try {
      ...
      if (server.type === "stdio" && server.sandboxUrl) {
        try {
          ...
          console.error(
            `Error stopping sandbox for server ${serverId}:`,
            error
          );
        }
      }
